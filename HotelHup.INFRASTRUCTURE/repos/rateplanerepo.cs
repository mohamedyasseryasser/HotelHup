...
namespace HotelHup.INFRASTRUCTURE.repos
{
    ...
    public sealed class RatePlanRepository : IRatePlanRepository
    {
            ...
            CancellationToken ct = default)
        {
            ...
            {
                await _context.RatePlansversions.AddAsync(
                    version,
                    ct);
            }
            ...
            try
            {
                // First save to generate database IDs and RowVersion
                await _context.SaveChangesAsync(ct);

                audit.TargetEntityId =
                    plan.Id.ToString();

                object? persistedNewValues =
                    audit.NewValues;

                if (version is not null)
                {
                    persistedNewValues = new
                    {
                        RatePlanId = plan.Id,
                    ...
                    new AuditLog
                    {
                        ...
                        OldValues = audit.OldValues is null
                            ...
                            : JsonSerializer.Serialize(
                                audit.OldValues),
                        NewValues = persistedNewValues is null
                            ? null
                            : JsonSerializer.Serialize(
                                persistedNewValues),
                        Reason = audit.Reason,
