...
public sealed class ReservationService : IReservationService
{
    ...
    public Task<ResponseStatus<ReservationResponse>>
        ...
        CancellationToken ct = default)
    {
        ...
        {
            var source = replacements.Count > 0 ? replacements : reservation.ReservationRooms.Where(x => x.IsCurrent).Select(x => new UpdateReservationRoomRequest { ReservationRoomId = x.Id, RoomId = x.RoomId, RatePlanId = x.RatePlanId, Adults = x.Adults, Children = x.Children, DiscountAmount = x.DiscountAmount, FeeAmount = x.FeeAmount }).ToList();
            if (source.Count == 0)
            {
                return Fail<ReservationResponse>("At least one room is required.", "ROOMS_REQUIRED", 422);
            }
            ...
            {
                old.IsCurrent = false; 
                old.ReleasedAt = DateTimeOffset.UtcNow;
            }
            ...
            if (reservation.Folio is not null)
            {
                ...
                reservation.Folio.Balance = reservation.TotalAmount;
            }
        }
        else 
        { 
    ...
    public Task<ResponseStatus<ReservationActionResponse>> CancelAsync(User actor, int propertyId, int id, CancelReservationRequest request, CancellationToken ct = default) =>
        TransitionAsync(actor, propertyId, id, ReservationStatus.Cancelled, request.Reason, request.ExpectedRowVersion, request.OverridePolicy, false, ct);

    public Task<ResponseStatus<ReservationActionResponse>> NoShowAsync(User actor, int propertyId, int id, NoShowRequest request, CancellationToken ct = default) =>
        TransitionAsync(actor, propertyId, id, ReservationStatus.NoShow, request.Reason ?? "No-show recorded", request.ExpectedRowVersion, request.OverridePolicy, true, ct);
    ...
    public async Task<ResponseStatus<ReservationActionResponse>> AssignRoomAsync(User actor, int propertyId, int id, AssignRoomRequest request, CancellationToken ct = default)
    {
        ...
        RecalculateTotals(reservation, taxes); reservation.Adults = reservation.ReservationRooms.Where(x => x.IsCurrent).Sum(x => x.Adults); reservation.Children = reservation.ReservationRooms.Where(x => x.IsCurrent).Sum(x => x.Children); reservation.UpdatedAt = now; reservation.UpdatedBy = actor.Id;
         ...
         CancellationToken ct)
    {
        ...
        {
            return Fail<ReservationActionResponse>(authorization.Message, 
                "FORBIDDEN", 
        ...
        if (property is null)
        {
            return Fail<ReservationActionResponse>("Property was not found.", "PROPERTY_NOT_FOUND", 404);
        }
        if (property.Status != PropertyStatus.Active)
        {
            return Fail<ReservationActionResponse>("Property is inactive.", "PROPERTY_INACTIVE", 409);
        }
        if (reservation.Guest is null || !reservation.Guest.IsActive)
        {
            return Fail<ReservationActionResponse>("Guest was not found or is inactive.", "GUEST_NOT_FOUND", 404);
        }
        ...
        if (!valid)
        {
            return Fail<ReservationActionResponse>("The requested status transition is not allowed.", "INVALID_STATE_TRANSITION", 409);
        }
        var now = DateTimeOffset.UtcNow;
        ...
        decimal cancellationFee = 0m, refundableAmount = 0m, heldAmount = 0m, noShowCharge = 0m;
        if (target == ReservationStatus.Confirmed)
        {
            ...
            var paidDeposit = reservation.Folio?.Payments.
                Where(x => x.Status is PaymentStatus.Paid or PaymentStatus.PartiallyPaid).Sum(x => x.Amount) ?? 0m;
            ...
            foreach (var assignment in reservation.ReservationRooms.Where(x => x.IsCurrent))
            {
                var available = await _repository.GetAvailableRoomsAsync(propertyId,
                    ...
                    assignment.Adults,
                    assignment.Children,
                    assignment.Room?.RoomTypeId,
                    id,
                    ct);
                if (!available.Any(x => x.Id == assignment.RoomId))
                {
                    return Fail<ReservationActionResponse>(
                        $"{assignment.RoomId} is not avaliable The selected room is no longer available .",
                        "ROOM_NO_LONGER_AVAILABLE",
                        409);
                }
            }
        ...
        else if (target == ReservationStatus.Cancelled)
        {
            ...
            if (reservation.CancellationPolicySnapshot is not null)
            {
                ...
                refundableAmount = Money(Math.Max(0m, reservation.TotalAmount - cancellationFee));
                heldAmount = cancellationFee;
                if (cancellationFee > 0m && reservation.Folio is not null)
                {
                    reservation.Folio.Items.Add(new FolioItem { Type = FolioItemType.Adjustment, Description = "Cancellation fee", Amount = cancellationFee, Source = FolioItemSource.System, Status = FolioItemStatus.Posted, PostedAt = now, CreatedAt = now, CreatedBy = actor.Id });
                    reservation.Folio.Total += cancellationFee;
                    reservation.Folio.Balance += cancellationFee;
                }
            }
        }
        else if (isNoShow)
        {
            ...
            if (isReceptionist && !isManager && !isAdmin && !overridePolicy && !isPastNoShowTime)
                return Fail<ReservationActionResponse>("No-show can only be recorded after the hotel cutoff time.", "NO_SHOW_CUTOFF_NOT_REACHED", 403);
            ...
            {
                reservation.Folio.Items.Add(new FolioItem { Type = FolioItemType.Adjustment, Description = "No-show charge", Amount = noShowCharge, Source = FolioItemSource.System, Status = FolioItemStatus.Posted, PostedAt = now, CreatedAt = now, CreatedBy = actor.Id });
                reservation.Folio.Total += noShowCharge;
                reservation.Folio.Balance += noShowCharge;
            }
        }
        ...
        var audit = new AuditLog
        {
            ...
            NewValues = JsonSerializer.Serialize(new { Status = target.ToString(), cancellationFee, refundableAmount, noShowCharge }),
            CreatedAt = now,
            CreatedBy = actor.Id
        };
        try { await _repository.PersistTransitionAsync(reservation, audit, ct); }
        catch (DbUpdateConcurrencyException) { return Fail<ReservationActionResponse>("The reservation was modified by another request.", "CONCURRENCY_CONFLICT", 409); }
        return Ok(Action(reservation, cancellationFee, refundableAmount, heldAmount, noShowCharge, reason));
    }
    ...
    private async Task<ReservationRoom?>
        BuildRoomLineAsync(decimal discount, decimal feeamount, int propertyId,
        Room room,
        int? ratePlanId,
        int adults,
        int children,
        ...
        CancellationToken ct)
    {
        ...
        var version = plan?.Versions.Where(x => x.IsActive
        && x.ValidFrom.Date <= checkIn.Date
        && (x.ValidTo == null || x.ValidTo.Value.Date > checkIn.Date)).
        OrderByDescending(x => x.VersionNumber).
        FirstOrDefault();
        if (plan is null || version is null) return null;
        var nights = (checkOut.Date - checkIn.Date).Days;
        var rate = version.Price > 0 ? version.Price : room.PricePerNight > 0 ? room.PricePerNight : room.RoomType?.BasePrice ?? 0m;
        var baseAmount = Money(rate * nights);
        var DiscountAmount = discount < 0 ? 0 : discount;
        var FeeAmount = feeamount < 0 ? 0 : feeamount;
        // Taxes are calculated once at Reservation level. A room line contains
        // only its net room charge: Base - Discount + Fee.
        var total = Money(baseAmount - DiscountAmount + FeeAmount);
    ...
    private static void
        RecalculateTotals(Reservation reservation,
        IReadOnlyList<Tax> taxes)
    {
        var lines = reservation.ReservationRooms.Where(x => x.IsCurrent).ToList();
        reservation.BaseAmount = Money(lines.Sum(x => x.BaseAmount));
        reservation.TotalDiscountAmount = Money(lines.Sum(x => x.DiscountAmount));
        reservation.TotalFeeAmount = Money(lines.Sum(x => x.FeeAmount));
        var taxableAmount = Money(reservation.BaseAmount - reservation.TotalDiscountAmount);
        reservation.TotalTaxAmount = Money(taxes.Sum(t => t.Type == TaxType.Percentage
    ...
    private async Task
        AddPolicySnapshotsAsync(Reservation reservation,
        CreateReservationRequest request,
        DateTime checkIn,
        DateTimeOffset now,
        CancellationToken ct)
    ...
    private static ReservationActionResponse Action(Reservation x) => new() { Id = x.Id, Status = x.Status, RowVersion = Convert.ToBase64String(x.RowVersion ?? Array.Empty<byte>()), RoomIds = x.ReservationRooms.Where(r => r.IsCurrent).Select(r => r.RoomId).ToList() };
    ...
    private static ReservationResponse
        Map(Reservation x)
        =>
        new()
        {
            ...
            Rooms = x.ReservationRooms.Select(
                r =>
